...
sub make_fixture : Tests(setup) {
    ...
    ok my $man = AcmeStore::Model::Manufacturer->new( name => 'foo' )->save,
      '... added manufacturer fixture';
    my $man_id = $man->id;

    ok my $cus = AcmeStore::Model::Customer->new(
        id         => 100,
        first_name => 'foo',
        last_name  => 'bar',
    )->save, '... added customer fixture';

    ok my $item = AcmeStore::Model::Item->new(
        name            => 'foofoo',
        price           => '$10',
...
sub order_number : Tests(no_plan) {
    my $test  = shift;
    my $class = $test->class;
