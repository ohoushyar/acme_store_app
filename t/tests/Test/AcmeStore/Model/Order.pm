...
sub make_fixture : Test(setup) {
    ...
    ok my $man_id = $schema->resultset('Manufacturer')->create({ name => 'foo' })->id,
...
sub order_number : Tests(4) {
    ...
    is $obj->order_number, '12-3', '... and order_number successfully returns the expected value';

    ok $obj->order_number('12-34'), '... and set the value successfully';
    is $obj->order_number, '12-34', '... and order_number successfully returns the expected value';
}
...
sub order_date : Tests(4) {
    ...
    is $obj->order_date, 1372536222, '... and order_date successfully returns the expected value';

    my $now = time;
    ok $obj->order_date($now), '... and set the value successfully';
    is $obj->order_date, $now, '... and order_date successfully returns the expected value';
}
...
sub customer_id : Tests(4) {
    ...
    is $obj->customer_id, 10, '... and customer_id successfully returns the expected value';

    ok $obj->customer_id(200), '... and set the value successfully';
    is $obj->customer_id, 200, '... and customer_id successfully returns the expected value';
}
...
sub save : Tests(no_plan) {
    ...
    ok my $result = $obj->save,
        '... and successfully saved Order';
